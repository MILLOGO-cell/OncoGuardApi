...
import cv2
import numpy as np
from app.core.config import TAGGED_DIR, UPLOAD_DIR
...
def create_tagged_image(src_path: str, base_filename: str, label: str, birads: str, confidence: float) -> str:
...
def create_and_predict_image_analysis(
    ...
    if not is_valid:
        raise HTTPException(status_code=400, detail=f"Format non supporté : {file.content_type} ({file_extension})")
    ...
    try:
        ...
        if file_extension in ['.dcm', '.dicom', '.pgm']:
            img_gray = convert_medical_image_to_grayscale(str(file_path))
            temp_png_path = UPLOAD_DIR / f"{uuid.uuid4()}_converted.png"
            cv2.imwrite(str(temp_png_path), img_gray)
            logger.info(f"Conversion en PNG : {temp_png_path}")
            prediction_path = str(temp_png_path)
        else:
            prediction_path = str(file_path)
        prediction = predict(prediction_path, confidence_threshold=confidence_threshold, uncertainty_threshold=uncertainty_threshold)
        birads_category = _label_to_birads(prediction.label)
        base_name = os.path.splitext(unique_filename)[0]
        tagged_filename = create_tagged_image(prediction_path, base_name, prediction.label, prediction.birads, float(prediction.confidence))
